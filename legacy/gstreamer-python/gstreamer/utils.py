...
#####################################################################################
def make_save_dir(cctv_id) -> str:
    ######## make dir ########
    print("start make_save_dir")
    playlist_dir:str ="/data/playlist/" 
    cctv_id = cctv_id.split("cctv")[-1]
    if len(cctv_id) == 1:
        cctv_id= f"0{str(cctv_id)}"
    else:
        cctv_id= f"{str(cctv_id)}"
    accident_dir=os.path.join(playlist_dir,"accident", cctv_id)
    streaming_dir=os.path.join(playlist_dir,"streaming", cctv_id)
    try:
        os.makedirs(accident_dir)
    except OSError:
    ...
    try:
        ...
        if not os.path.isdir(streaming_dir):
            raise
    return streaming_dir, accident_dir
    ###########################
    
def fraction_to_str(fraction: Fraction) -> str:                     # str 분수로 리턴
